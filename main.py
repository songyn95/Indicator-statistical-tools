...
def parse_opt():
    ...
    parser.add_argument("--output_file", type=str, default=ROOT / "result.csv", help="Statistical Results Table, Threshold 0-1")
...
def main(opt):
    ...
    ##abs path
    opt.source_file = opt.source_file.replace(".\\", os.getcwd() + os.sep, 1) if str(opt.source_file).startswith('.\\') else opt.source_file
    opt.Manually_annotate_dir = opt.Manually_annotate_dir.replace(".\\", os.getcwd() + os.sep, 1) if str(opt.Manually_annotate_dir).startswith('.\\') else opt.Manually_annotate_dir
    # to global path
    print(opt.source_file)

    #3. 获取yaml 里面数据路径 output log日志打印/存储
